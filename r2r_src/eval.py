...
class Evaluation(object):
    ...
    def cls(self, scan, prediction, reference):
        ...
        # for u in reference:
        #    print(-np.min([self.distances[scan][u][v] for v in prediction]) / self.error_margin)
        #    time.sleep(0.2)
    ...
    def score_fail(self, output_file):
        ''' Evaluate each agent trajectory based on how close it got to the goal location '''
        self.scores = defaultdict(list)
        instr_ids = set(self.instr_ids)
        if type(output_file) is str:
        ...
        for item in results:
            ...
            if item['instr_id'] in instr_ids:
                ...
                # self._score_item(item['instr_id'], item['trajectory'])
                self._score_item(item['instr_id'], item['trajectory'])
        fail_idx = [i for i in self.scores['nav_errors'] if i > self.error_margin]
        score_summary = {
            'nav_error': np.average(self.scores['nav_errors']),
            'oracle_error': np.average(self.scores['oracle_errors']),
            'steps': np.average(self.scores['trajectory_steps']),
            'lengths': np.average([self.scores['trajectory_lengths'][i] for i in fail_idx]),
            'cls': np.average(self.scores['cls']),
            'ndtw': float(sum(self.scores['ndtw']) / len(self.scores['ndtw'])),
            'sdtw': float(sum(self.scores['sdtw']) / len(self.scores['sdtw']))
        }
    ...
    def fail_idx(self, output_file):
        ...
        for item in results:
            ...
            if item['instr_id'] in instr_ids:
                ...
                self._score_item(item['instr_id'], item['trajectory'])
        fail_idx = []
        for i, (ne, ora) in enumerate(zip(self.scores['nav_errors'], self.scores['oracle_errors'])):
            if ne > self.error_margin:
                fail_idx.append(i)
        return fail_idx

    def fidelity(self, output_file, idx):
        self.scores = defaultdict(list)
        if type(output_file) is str:
            with open(output_file) as f:
                results = json.load(f)
        ...
        ndtw = 0
        cls = 0
        val_unseen_dataset = load_datasets(['val_unseen'])
        num_test = 0
        for i, item in enumerate(results):
            ...
            if i in idx:
                num_test +=1
                # check which scan this path belongs to
                path_id = item['instr_id'].split("_")[0]
                item_scan = ''
                for it in val_unseen_dataset:
                    if it['path_id'] == int(path_id):
                        item_scan = it['scan']
                        break
                for key, gt in self.gt.items():
                    if item_scan == gt['scan']:
                        if self.ndtw(gt['scan'], item['trajectory'], gt['path']) > max_ndtw:
                            max_ndtw = self.ndtw(gt['scan'], item['trajectory'], gt['path'])
                        if self.cls(gt['scan'], item['trajectory'], gt['path']) > max_cls:
                            max_cls = self.cls(gt['scan'], item['trajectory'], gt['path'])
                # print(max_ndtw, max_cls)
                ndtw += max_ndtw
                cls += max_cls
        return ndtw / num_test, cls / num_test

    def success_case(self, output_file):
        self.scores = defaultdict(list)
        instr_ids = set(self.instr_ids)
        if type(output_file) is str:
            with open(output_file) as f:
        ...
        for item in results:
            ...
            if item['instr_id'] in instr_ids:
                ...
                self._score_item(item['instr_id'], item['trajectory'])
        success_idx = []
        for i, (ne, ndtw) in enumerate(zip(self.scores['nav_errors'], self.scores['ndtw'])):
            if ne < self.error_margin:
                success_idx.append(i) # results[i]['instr_id'])
