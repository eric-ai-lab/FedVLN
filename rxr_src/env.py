    ...
    def __init__(self, feature_store, batch_size=100, seed=10, splits=['train'], tokenizer=None,
        ...
        for split in splits:
            for item in load_datasets([split]):
                ...
                if tokenizer:
                    if new_item['language'] == 'en':
                        instr = item['instruction']
                        new_item['instr_encoding'] = tokenizer.encode_sentence(instr, new_item['language'])
                    else:
                        assert len(item['parse_tree_origin']) == 1
                        parse = item['parse_tree_origin'][0]
                        new_item['instr_encoding'] = tokenizer.encode_sentence(parse, new_item['language'])
...
class R2RBatchScan():
    ...
    def __init__(self, feature_store, batch_size=100, seed=10, splits=['train'], tokenizer=None,
        ...
        for split in splits:
            for item in load_datasets([split]):
                if item['scan'] not in self.data:
                    self.data[item['scan']] = []
                    self.scans_list.append(item['scan'])
                if item['scan'] not in self.env.featurized_scans:  # For fast training
                    continue
                new_item = dict(item)
                new_item['instr_id'] = item['instruction_id']
                ...
                if tokenizer:
                    if new_item['language'] == 'en':
                        ...
                        new_item['instr_encoding'] = tokenizer.encode_sentence(parse, new_item['language'])
                if not tokenizer or new_item['instr_encoding'] is not None:  # Filter the wrong data
                    self.data[item['scan']].append(new_item)
                    scans.append(item['scan'])
        ...
        # It means that the fake data is equals to data in the supervised setup
        self.fake_data = self.data
        print('R2RBatch loaded with %d instructions, using splits: %s' % (len(self.data), ",".join(splits)), self.size(), add_time)

    def size(self):
        length = 0
        for scan in self.data:
            length += len(self.data[scan])
        return length
    ...
    def _next_minibatch(self, tile_one=False, batch_size=None, **kwargs):
        ...
        if tile_one:
            batch = [self.data[self.ix]] * batch_size
            self.ix += 1
            if self.ix >= len(self.data[self.current_scan]):
                random.shuffle(self.data[self.current_scan])
                self.ix -= len(self.data[self.current_scan])
        else:
            batch = self.data[self.current_scan][self.ix: self.ix+batch_size]
            if len(batch) < batch_size:
                while len(batch) < batch_size:
                    random.shuffle(self.data[self.current_scan])
                    self.ix = batch_size - len(batch)
                    batch += self.data[self.current_scan][:self.ix]
            else:
                self.ix += batch_size
            self.batch = batch
    ...
    def _get_obs(self):
        ...
        for i, (feature, state) in enumerate(self.env.getStates()):
            try:
                ...
                print(i, len(self.batch), len(self.data[self.current_scan]))
    ...
    def set_current_scan(self, scan):
        if type(scan) is str:
            ...
            self.scan_idx = self.scans_list.index(scan)
        else:
            self.current_scan = self.scans_list[scan]
            self.scan_idx = scan
