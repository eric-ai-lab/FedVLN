...
def load_datasets(splits):
    ...
    for split in splits:
        ...
        if "/" not in split:
            ...
            with open('tasks/RxR/data/rxr_%s_guide_dep_all.jsonl' % split) as f:
                new_data = json.load(f)
        else:
...
class Tokenizer(object):
    ...
    def encode_sentence(self, sentence, language, max_length=None):
        ...
        if language == 'en':
            ...
            for i in range(1, len(sentence) + 1):
                encoding.append(self.word_to_index[sentence[str(i)]['word']])
...
def build_vocab_te(splits=['train'], min_count=5, start_vocab=base_vocab):
    count = Counter()
    data = load_datasets(splits)
...
def build_vocab_hi(splits=['train'], min_count=5, start_vocab=base_vocab):
    count = Counter()
    data = load_datasets(splits)
...
def viewpoint_drop_mask(viewpoint, seed=None, drop_func=None):
    ...
    drop_mask = drop_func(torch.ones(512).cuda())
    return drop_mask
